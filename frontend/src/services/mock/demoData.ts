...
export function localInsight(scenario: DemoScenario, serverName = "complex-server-01"): IncidentInsight {
  ...
  return {
    ...
    summary: `${serverName}의 응답시간이 평소 ${baselineResponseMs}ms에서 ${currentResponseMs}ms로 늘었고, 최근 수집 구간에 Timeout ${timeoutCount}건과 오류 ${errorCount}건이 관찰되었습니다. ${latency ? "응답 지연과 시간 초과가 함께 나타나 연결 대기 상태를 우선 확인해야 합니다." : "오류가 집중되어 서비스 연동 요청과 응답 형식을 우선 확인해야 합니다."}`,
