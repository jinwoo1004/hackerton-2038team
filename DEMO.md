...
3. 프로젝트 목록에서 개포 프레지던스 자이 또는 북수원 자이 렉스비아를 열어 분석 결과·합성 소스 및 규칙 근거를 확인한다.
4. 모니터링에서 CPU·메모리·디스크와 단지별 응답시간 그래프를 확인한다. 기간을 1시간/6시간/24시간으로 바꿀 수 있다.
5. **응답 지연 트리거 → AI 설명 → Slack 미리보기 → 정상 복구**를 실행한다. 시연 데이터 초기화는 준비된 샘플을 복원한다.

발표 시 “이 화면은 단지 운영 흐름을 재현한 합성 데이터이며 실제 운영 장애나 실제 AI 생성 결과가 아닙니다”라고 설명한다. 외부 알림 발송과 유료 모델 호출은 발생하지 않는다. 각 방문자의 설정은 해당 브라우저에만 저장된다. 실제 첨부 소스·로그는 배포하지 않는다.
