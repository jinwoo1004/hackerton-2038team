...
## 현재 기본: Vercel 프런트엔드 시연

백엔드·DB·Agent·AI 로그인 없이 웹 화면만 시연한다. `frontend`에서 `npm ci`, `npm run build`, `npm start`로 실행한다. 기본값은 `NEXT_PUBLIC_DATA_MODE=mock`이며 Vercel도 동일하다. 배포 설정은 [배포 안내](docs/DEPLOYMENT.md)의 첫 절을 따른다.

1. 로그인 화면의 **데모 둘러보기**를 누른다.
2. 대시보드에서 7개 시연 프로젝트, 30일 분석 이력, 품질 추이와 7일 장애 추이를 확인한다.
3. 프로젝트 목록에서 수목관리플랫폼 또는 워크허브를 열어 분석 결과·합성 소스 및 규칙 근거를 확인한다.
4. 모니터링에서 CPU·메모리·디스크와 시스템별 서비스 응답시간 그래프를 확인한다. 기간을 1시간/6시간/24시간으로 바꿀 수 있다.
5. **응답 지연 트리거 → AI 설명 → Slack 미리보기 → 정상 복구**를 실행한다. 시연 데이터 초기화는 준비된 샘플을 복원한다.

발표 시 “이 화면은 여러 사내 시스템의 운영 흐름을 재현한 합성 데이터이며 실제 운영 장애나 실제 AI 생성 결과가 아닙니다”라고 설명한다. 외부 알림 발송과 유료 모델 호출은 발생하지 않는다. 각 방문자의 설정은 해당 브라우저에만 저장된다. 실제 첨부 소스·로그는 배포하지 않는다.

아래 절차는 실제 백엔드·Agent 연결까지 보여주는 **선택적 전체 스택** 시연용이다.
